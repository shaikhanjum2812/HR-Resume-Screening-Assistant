...
from interview_engine_new import InterviewEngine  # Using the new interview engine
...
from interview_ui import show_interviews as show_new_interviews  # Import the new interview UI
...
def show_evaluation():
    ...
    if uploaded_files:
        ...
        if selected_job and st.button("Start Evaluation"):
            ...
            success_count = 0
            evaluation_results = []  # Store evaluation results
            ...
            if success_count > 0:
                st.write("#### Download Summary Report")
                summary_buffer = generate_summary_report(evaluation_results)
                st.download_button(
                    label="📄 Download Shortlisted Candidates Summary (PDF)",
                    data=summary_buffer,
                    file_name=f"shortlisted_candidates_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                    mime="application/pdf"
                )
...
def init_session_state():
    ...
    # Initialize session state for the new interview UI
    if 'interview_mode' not in st.session_state:
        st.session_state.interview_mode = "setup"
...
def show_interviews():
    """Wrapper for the new interview system UI
    
    This replaces the old SAP-specific interview interface with the
    new generic interactive interview system
    """
    # Import the interview UI module
    import interview_ui
    
    # Ensure database tables are ready
    if 'db_updated' not in st.session_state or not st.session_state.db_updated:
        try:
            import database_update
            success = database_update.update_database_schema()
            st.session_state.db_updated = success
            if not success:
                st.error("Failed to update database schema for the interview system")
                st.info("The new interview system requires database updates. Please contact support.")
                return
        except Exception as e:
            logger.error(f"Error updating database schema: {str(e)}")
            st.error(f"Error initializing the interview system: {str(e)}")
            return
    
    # Call the new interview UI component
    interview_ui.show_interviews()
def sidebar():
